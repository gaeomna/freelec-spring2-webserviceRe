...
import com.first.book.springboot.config.auth.SecurityConfig;
...
@WebMvcTest
: 여러 스프링 어노테이션 중,Web(Spring MVC)에 집중 할수 있는 어노테이션.
선언시 @Controller,@ControllerAdvice등 사용가능
단@service @Component @Repository등은 사용x
지금은 Comtroller만 사용하기에 사용

private MockMvcmvc;
: 웹API를 테스트할 때 사용.
스프링 MVC 테스트의 시작점.
이 class를 통해 HTTP GET, POST 등에 대한 API 테스트 가능

@Autowired
: 스프링이 관리하는 빈(Bean)을 주입 받음

mvc.perform(get("/hello"))
: MockMvc를 통해 /hello주소로 HTTP GET요청을 합니다.
...
Param
: API테스트 할 때 사용될 요청 파라미터를 설정.단 값은 String만 허용,그래서 숫자/날짜 등의 데이터도 등록시에는 문자열로 변경해야함

jsonPath
: JSON응답값을 필드별로 검증할 수 있는 메소드.
$를 기준으로 필드명을 명시.
여기서는 name과amount를 검증하니 $.name $.amount로 검증
